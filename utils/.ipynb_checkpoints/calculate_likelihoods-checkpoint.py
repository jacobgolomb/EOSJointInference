...
import h5py
import sys
sys.path.append('/home/jacob.golomb/GWs_and_pulsars/utils/')
...
samples['chirp_mass_source'] = component_masses_to_chirp_mass(m1s, m2s)
samples['mass_ratio'] = m2s/m1s

def sample_masses_and_lambdas(N, params, eos):
    l1s, l2s = lambdas[eos]
    samples['lambda_1'] = l1s
    samples['lambda_2'] = l2s
    return samples

def dummy_selection(params):
    return 1

full_likelihood = GMMLikelihood(gmm_files= gw_gmm_files, 
                         population_sample_func = sample_masses_and_lambdas, selection_function = dummy_selection, 
